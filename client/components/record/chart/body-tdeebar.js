...
export default function BodyTdeeChart({ endDate, setTDEE }) {
  ...
  useEffect(() => {
    if (dataDetail.length > 0) {
      // ac對照表
      const ac = { ac1: 1.2, ac2: 1.375, ac3: 1.55, ac4: 1.725, ac5: 1.9 }

      // 計算TDEE
      const BMR =
        (10 * dataDetail[0].Weight) + (6.25 * dataDetail[0].Height) - (5 * 30) + 5
      const TDEE = parseFloat(
        BMR * ac[dataDetail[0].Activity_coefficient]
      ).toFixed(1)
      setTDEE(TDEE)
